...
### ⚠️ draw.io MCP/CLI SVG 导出不渲染 edge（已知限制）

draw.io 的 SVG 导出（无论 MCP 还是 CLI `drawio --export --format svg`）**不渲染 edge 的路径**。导出的 SVG 中，所有 edge 都是空的 `<g data-cell-id="e1"/>` 标签，没有 `<path>` 元素。这是因为 edge 路径的计算依赖 mxGraph JavaScript 运行时，静态导出时丢失。

**解决方案：当需要在 GitHub README 等静态场景使用时，直接手写 SVG XML，不经过 draw.io 导出。**

手写 SVG 中的 edge 渲染方式（每条 edge 需要两个 `<path>`）：
```xml
<!-- 线条 -->
<path d="M 306 135 L 306 162 Q 306 172 306 172 L 306 202"
  fill="none" stroke="#78909c" stroke-width="2"/>
<!-- 箭头（三角形闭合路径） -->
<path d="M 306 208 L 302 200 L 306 202 L 310 200 Z"
  fill="#78909c" stroke="#78909c" stroke-width="2"/>
```

正交（L 形）连线用 Q（quadratic curve）实现圆角转弯：
```xml
...
画完图后自查（**按顺序检查**）：
- [ ] **edge 连线数量 > 0**（如果为 0，100% 是遗漏了，必须补上）
- [ ] **所有相邻节点之间都有 edge 连线**（最重要！数一下 edge 数量，应接近节点数）
